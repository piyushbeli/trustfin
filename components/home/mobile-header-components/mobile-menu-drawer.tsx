...
import { AUTH_NAV_LINKS } from '@/lib/constants/mobile-nav';
import { sortHeaderLinks } from '@/lib/navigation/header-links';
...
export const MobileMenuDrawer = ({
  ...
  const sortedHeaderLinks = sortHeaderLinks(headerLinks ?? []);
  const sortedAuthLinks = isAuthenticated ? sortHeaderLinks(AUTH_NAV_LINKS) : [];
  const hasNavLinks = sortedAuthLinks.length > 0 || sortedHeaderLinks.length > 0;
  ...
  return (
    <>
      ...
      <div className="fixed top-0 left-0 bottom-0 w-[280px] z-50 bg-wc-dark shadow-2xl">
        ...
        {/* Navigation: auth-only links, then public header links */}
        {hasNavLinks && (
          <nav className="p-4">
            <ul className="space-y-1">
              {sortedAuthLinks.map((link) => renderNavListItem(link, closeMenu))}
              {sortedHeaderLinks.map((link) => renderNavListItem(link, closeMenu))}
            </ul>
          </nav>
        )}
      </div>
